...
class AIClient:
    ...
    def __init__(self, config: AppConfig):
        ...
        if config.ollama_base_url:
            ...
            logger.info(
                f"AI client initialized with Ollama (%s @ %s, ~%.1fs/req)",
                self.MODELS[0],
                base_url,
            ...
            logger.info(
                f"AI client initialized with Groq (%s models, ~%ds/req)",
                len(self.MODELS),
                self.rate_limit_delay,
            )
            ...
            logger.info(
                f"AI client initialized with OpenRouter (%s models, ~%ds/req)",
                len(self.MODELS),
                self.rate_limit_delay,
            )
    ...
    def _call(self, prompt: str, max_tokens: int = 2000) -> str:
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                self._current_model_idx = (self._current_model_idx + 1) % len(
                    self.MODELS
                )
                ...
                if self._using_ollama:
                    ...
                    logger.warning(
                        f"[{self._provider_name}] Model {model} unavailable (404) — skipping, switching to {next_model}"
                    )
                elif isinstance(e, RateLimitError):
                    logger.warning(
                        f"[{self._provider_name}] Rate limited (429) on {model} — switching to {next_model}"
                    )
                else:
                    logger.warning(
                        f"[{self._provider_name}] Connection error on {model}: {e} — switching to {next_model}"
                    )
                ...
                if attempt >= len(self.MODELS):
                    ...
                    logger.warning(
                        f"[{self._provider_name}] All models exhausted, backing off {delay:.0f}s before retry..."
                    )
                    time.sleep(delay)
        ...
        logger.error(
            f"[{self._provider_name}] All API attempts failed after {max_retries} retries"
        )
        if self._using_ollama:
            raise RuntimeError(
    ...
    def generate_cv(
        ...
        for j in jobs:
            ...
            job_summaries.append(
                f"- {j.get('title', 'Unknown')} at {j.get('company', 'Unknown')}: {desc_preview}"
            )
        jobs_context = "\n".join(job_summaries)
...
Return ONLY the CV text, properly formatted with clear sections. Do not include any meta-commentary."""
    ...
    def analyze_resume_for_keywords(
        self, resume_text: str, current_roles: Optional[List[str]] = None
    ) -> List[str]:
        """
        ...
        if not self.is_available:
            raise EnvironmentError(
                "No API key configured. Set GROQ_API_KEY or OPENROUTER_API_KEY"
            )

        current_roles_str = (
            ", ".join(current_roles) if current_roles else "Not specified"
        )
...
Do not include any other text or explanation."""
    ...
    def score_by_title_only(
        self, profile: Dict[str, Any], job: Job, resume_text: str = ""
    ) -> AIResult:
        ...
        _fb_section = (
            ...
            if getattr(self, "feedback_insights", "")
            else ""
        )
