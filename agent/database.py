...
def get_user_by_id(user_id: int) -> Optional[Dict[str, Any]]:
    conn = get_db()
    cur = _cursor(conn)
    cur.execute("SELECT * FROM users WHERE id = %s" if _use_postgres else "SELECT * FROM users WHERE id = ?", (user_id,))
    row = cur.fetchone()
...
def get_user_by_email(email: str) -> Optional[Dict[str, Any]]:
    conn = get_db()
    cur = _cursor(conn)
    cur.execute("SELECT * FROM users WHERE LOWER(email) = LOWER(%s)" if _use_postgres else "SELECT * FROM users WHERE LOWER(email) = LOWER(?)", (email,))
    row = cur.fetchone()
    return _row_to_dict(row) if row else None
...
def get_all_users() -> List[Dict[str, Any]]:
    conn = get_db()
    cur = _cursor(conn)
    cur.execute("SELECT id, email, name, role, status, password_changed, created_at FROM users ORDER BY created_at DESC")
...
def approve_user(user_id: int) -> bool:
    conn = get_db()
    cur = _cursor(conn)
    cur.execute("UPDATE users SET status = 'active', role = 'user' WHERE id = %s AND status = 'pending'" if _use_postgres else "UPDATE users SET status = 'active', role = 'user' WHERE id = ? AND status = 'pending'", (user_id,))
    conn.commit()
...
def reject_user(user_id: int) -> bool:
    conn = get_db()
    cur = _cursor(conn)
    cur.execute("DELETE FROM users WHERE id = %s AND status = 'pending'" if _use_postgres else "DELETE FROM users WHERE id = ? AND status = 'pending'", (user_id,))
    conn.commit()
    return cur.rowcount > 0
...
def get_pending_users() -> List[Dict[str, Any]]:
    conn = get_db()
    cur = _cursor(conn)
    cur.execute("SELECT id, email, name, created_at FROM users WHERE status = 'pending' ORDER BY created_at ASC")
    return [_row_to_dict(r) for r in cur.fetchall()]
...
def update_user_api_key(user_id: int, api_key: str):
    conn = get_db()
    cur = _cursor(conn)
    cur.execute("UPDATE users SET api_key = %s WHERE id = %s" if _use_postgres else "UPDATE users SET api_key = ? WHERE id = ?", (api_key, user_id))
    conn.commit()
...
def update_user_resend_key(user_id: int, resend_key: str):
    conn = get_db()
    cur = _cursor(conn)
    cur.execute("UPDATE users SET resend_api_key = %s WHERE id = %s" if _use_postgres else "UPDATE users SET resend_api_key = ? WHERE id = ?", (resend_key, user_id))
    conn.commit()
    return True
...
def update_gmail_credentials(user_id: int, gmail_user: str, gmail_app_password: str):
    ...
    cur = _cursor(conn)
    cur.execute(
        "UPDATE users SET gmail_user = %s, gmail_app_password = %s WHERE id = %s" if _use_postgres else "UPDATE users SET gmail_user = ?, gmail_app_password = ? WHERE id = ?",
...
def update_user_role(user_id: int, role: str) -> bool:
    conn = get_db()
    cur = _cursor(conn)
    cur.execute("UPDATE users SET role = %s WHERE id = %s" if _use_postgres else "UPDATE users SET role = ? WHERE id = ?", (role, user_id))
    conn.commit()
    return cur.rowcount > 0
...
def update_user_status(user_id: int, status: str) -> bool:
    conn = get_db()
    cur = _cursor(conn)
    cur.execute("UPDATE users SET status = %s WHERE id = %s" if _use_postgres else "UPDATE users SET status = ? WHERE id = ?", (status, user_id))
    conn.commit()
    return cur.rowcount > 0
...
def delete_user(user_id: int):
    conn = get_db()
    cur = _cursor(conn)
    cur.execute("DELETE FROM users WHERE id = %s" if _use_postgres else "DELETE FROM users WHERE id = ?", (user_id,))
    conn.commit()
...
def update_user_password(user_id: int, password_hash: str) -> bool:
    ...
    cur = _cursor(conn)
    cur.execute(
        "UPDATE users SET password_hash = %s WHERE id = %s" if _use_postgres else "UPDATE users SET password_hash = ? WHERE id = ?",
...
def update_user_email(user_id: int, email: str) -> bool:
    ...
    cur = _cursor(conn)
    try:
        cur.execute("UPDATE users SET email = %s WHERE id = %s" if _use_postgres else "UPDATE users SET email = ? WHERE id = ?", (email, user_id))
        conn.commit()
        return cur.rowcount > 0
    except Exception:
        conn.rollback() if _use_postgres else None
...
def update_user_name(user_id: int, name: str) -> bool:
    conn = get_db()
    cur = _cursor(conn)
    cur.execute("UPDATE users SET name = %s WHERE id = %s" if _use_postgres else "UPDATE users SET name = ? WHERE id = ?", (name, user_id))
    conn.commit()
    return cur.rowcount > 0
...
def mark_password_changed(user_id: int) -> bool:
    conn = get_db()
    cur = _cursor(conn)
    cur.execute("UPDATE users SET password_changed = 1 WHERE id = %s" if _use_postgres else "UPDATE users SET password_changed = 1 WHERE id = ?", (user_id,))
    conn.commit()
    return cur.rowcount > 0
...
def mark_password_needs_change(user_id: int) -> bool:
    conn = get_db()
    cur = _cursor(conn)
    cur.execute("UPDATE users SET password_changed = 0 WHERE id = %s" if _use_postgres else "UPDATE users SET password_changed = 0 WHERE id = ?", (user_id,))
    conn.commit()
    return cur.rowcount > 0
...
def needs_password_change(user_id: int) -> bool:
    conn = get_db()
    cur = _cursor(conn)
    cur.execute("SELECT password_changed FROM users WHERE id = %s" if _use_postgres else "SELECT password_changed FROM users WHERE id = ?", (user_id,))
    row = cur.fetchone()
    if row is None:
        return False
    d = _row_to_dict(row)
...
def log_activity(user_id: int, email: str, action: str, details: str = "", ip_address: str = ""):
    ...
    cur = _cursor(conn)
    cur.execute(
        "INSERT INTO activity_log (user_id, email, action, details, ip_address) VALUES (%s, %s, %s, %s, %s)" if _use_postgres else "INSERT INTO activity_log (user_id, email, action, details, ip_address) VALUES (?, ?, ?, ?, ?)",
...
def get_user_activity(user_id: int, limit: int = 50) -> List[Dict[str, Any]]:
    ...
    cur = _cursor(conn)
    cur.execute(
        "SELECT * FROM activity_log WHERE user_id = %s ORDER BY created_at DESC LIMIT %s" if _use_postgres else "SELECT * FROM activity_log WHERE user_id = ? ORDER BY created_at DESC LIMIT ?",
        (user_id, limit),
    )
    return [_row_to_dict(r) for r in cur.fetchall()]
...
def get_all_recent_activity(limit: int = 100) -> List[Dict[str, Any]]:
    ...
    cur = _cursor(conn)
    cur.execute(
        "SELECT a.*, u.name as user_name FROM activity_log a LEFT JOIN users u ON a.user_id = u.id ORDER BY a.created_at DESC LIMIT %s" if _use_postgres else "SELECT a.*, u.name as user_name FROM activity_log a LEFT JOIN users u ON a.user_id = u.id ORDER BY a.created_at DESC LIMIT ?",
        (limit,),
    )
    return [_row_to_dict(r) for r in cur.fetchall()]
...
def get_user_activity_stats(user_id: int) -> Dict[str, Any]:
    ...
    cur = _cursor(conn)
    cur.execute(
        "SELECT COUNT(*) as c FROM activity_log WHERE user_id = %s AND action = 'login'" if _use_postgres
        else "SELECT COUNT(*) as c FROM activity_log WHERE user_id = ? AND action = 'login'",
        (user_id,),
    )
    logins = _row_to_dict(cur.fetchone())["c"]
    cur.execute(
        "SELECT COUNT(*) as c FROM activity_log WHERE user_id = %s AND action = 'save_job'" if _use_postgres
        else "SELECT COUNT(*) as c FROM activity_log WHERE user_id = ? AND action = 'save_job'",
        (user_id,),
    )
    saves = _row_to_dict(cur.fetchone())["c"]
    cur.execute(
        "SELECT created_at FROM activity_log WHERE user_id = %s ORDER BY created_at DESC LIMIT 1" if _use_postgres
...
def get_active_users_count(minutes: int = 30) -> int:
    ...
    cur.execute(
        "SELECT COUNT(DISTINCT user_id) as c FROM activity_log WHERE created_at > %s" if _use_postgres else "SELECT COUNT(DISTINCT user_id) as c FROM activity_log WHERE created_at > ?",
        (cutoff,),
    )
    row = cur.fetchone()
...
def save_feedback(user_id: int, rating: int, job_title: str = "", company: str = "", application_id: int = None):
    ...
    cur = _cursor(conn)
    cur.execute(
        "INSERT INTO job_feedback (user_id, application_id, job_title, company, rating) VALUES (%s, %s, %s, %s, %s)" if _use_postgres else "INSERT INTO job_feedback (user_id, application_id, job_title, company, rating) VALUES (?, ?, ?, ?, ?)",
...
def create_password_reset_token(user_id: int) -> Optional[str]:
    ...
    cur = _cursor(conn)
    cur.execute(
        "DELETE FROM password_reset_tokens WHERE user_id = %s AND used = 0" if _use_postgres else "DELETE FROM password_reset_tokens WHERE user_id = ? AND used = 0",
        (user_id,),
    )
    conn.commit()
    ...
    cur.execute(
        "INSERT INTO password_reset_tokens (user_id, token, expires_at) VALUES (%s, %s, %s)" if _use_postgres else "INSERT INTO password_reset_tokens (user_id, token, expires_at) VALUES (?, ?, ?)",
...
def get_user_by_reset_token(token: str) -> Optional[Dict[str, Any]]:
    ...
    cur = _cursor(conn)
    cur.execute(
        "SELECT user_id FROM password_reset_tokens WHERE token = %s AND used = 0 AND expires_at > %s" if _use_postgres else "SELECT user_id FROM password_reset_tokens WHERE token = ? AND used = 0 AND expires_at > ?",
...
def use_password_reset_token(token: str) -> bool:
    ...
    cur = _cursor(conn)
    cur.execute(
        "UPDATE password_reset_tokens SET used = 1 WHERE token = %s" if _use_postgres else "UPDATE password_reset_tokens SET used = 1 WHERE token = ?",
        (token,),
    )
    conn.commit()
    return cur.rowcount > 0
...
def cleanup_expired_tokens():
    ...
    cur = _cursor(conn)
    cur.execute(
        "DELETE FROM password_reset_tokens WHERE expires_at < %s" if _use_postgres else "DELETE FROM password_reset_tokens WHERE expires_at < ?",
...
def log_login_attempt(
    ...
    cur = _cursor(conn)
    cur.execute(
        "INSERT INTO login_logs (user_id, email, ip_address, user_agent, success, details) VALUES (%s, %s, %s, %s, %s, %s)" if _use_postgres else "INSERT INTO login_logs (user_id, email, ip_address, user_agent, success, details) VALUES (?, ?, ?, ?, ?, ?)",
...
def get_login_logs(limit: int = 200) -> List[Dict[str, Any]]:
    ...
    cur = _cursor(conn)
    cur.execute(
        "SELECT l.*, u.name as user_name, u.role as user_role FROM login_logs l LEFT JOIN users u ON l.user_id = u.id ORDER BY l.created_at DESC LIMIT %s" if _use_postgres else "SELECT l.*, u.name as user_name, u.role as user_role FROM login_logs l LEFT JOIN users u ON l.user_id = u.id ORDER BY l.created_at DESC LIMIT ?",
        (limit,),
    )
    return [_row_to_dict(r) for r in cur.fetchall()]
...
def get_login_logs_for_user(user_id: int, limit: int = 50) -> List[Dict[str, Any]]:
    ...
    cur = _cursor(conn)
    cur.execute(
        "SELECT * FROM login_logs WHERE user_id = %s ORDER BY created_at DESC LIMIT %s" if _use_postgres else "SELECT * FROM login_logs WHERE user_id = ? ORDER BY created_at DESC LIMIT ?",
        (user_id, limit),
    )
    return [_row_to_dict(r) for r in cur.fetchall()]
...
def save_application(user_id: int, app_data: Dict[str, Any]) -> int:
    ...
    if _use_postgres:
        cur.execute(
            ...
            (
                user_id, app_data.get("timestamp", ""), app_data.get("title", "Unknown"),
                app_data.get("company", "Unknown"), app_data.get("url", ""),
                app_data.get("platform", ""), app_data.get("location", ""),
                app_data.get("ai_score", 0), skills_json, concerns_json,
                app_data.get("cover_letter", ""), app_data.get("job_description", ""),
            ),
        )
...
def get_all_applications(limit: int = 500) -> List[Dict[str, Any]]:
    ...
    cur = _cursor(conn)
    cur.execute(
        "SELECT a.*, u.email as user_email, u.name as user_name FROM applications a JOIN users u ON a.user_id = u.id ORDER BY a.timestamp DESC LIMIT %s" if _use_postgres else "SELECT a.*, u.email as user_email, u.name as user_name FROM applications a JOIN users u ON a.user_id = u.id ORDER BY a.timestamp DESC LIMIT ?",
        (limit,),
    )
    return [_parse_app_row(_row_to_dict(r)) for r in cur.fetchall()]
...
def delete_application(application_id: int) -> bool:
    conn = get_db()
    cur = _cursor(conn)
    cur.execute("DELETE FROM applications WHERE id = %s" if _use_postgres else "DELETE FROM applications WHERE id = ?", (application_id,))
    conn.commit()
    return cur.rowcount > 0
...
def clear_user_applications(user_id: int):
    ...
    cur = _cursor(conn)
    cur.execute(
        "DELETE FROM applications WHERE user_id = %s" if _use_postgres else "DELETE FROM applications WHERE user_id = ?",
        (user_id,),
    )
    cur.execute(
        "DELETE FROM saved_jobs WHERE user_id = %s" if _use_postgres else "DELETE FROM saved_jobs WHERE user_id = ?",
        (user_id,),
    )
    cur.execute(
        "DELETE FROM applied_jobs WHERE user_id = %s" if _use_postgres else "DELETE FROM applied_jobs WHERE user_id = ?",
        (user_id,),
    )
    conn.commit()
...
def mark_applied(user_id: int, url: str) -> bool:
    ...
    try:
        cur.execute(
            "INSERT INTO applied_jobs (user_id, url) VALUES (%s, %s)" if _use_postgres else "INSERT INTO applied_jobs (user_id, url) VALUES (?, ?)",
            (user_id, url),
        )
        conn.commit()
        return True
...
def get_applied_urls(user_id: int) -> set:
    conn = get_db()
    cur = _cursor(conn)
    cur.execute("SELECT url FROM applied_jobs WHERE user_id = %s" if _use_postgres else "SELECT url FROM applied_jobs WHERE user_id = ?", (user_id,))
...
def save_job_with_data(user_id: int, job_data: Dict[str, Any]) -> Optional[int]:
    ...
    try:
        cur.execute(
            "INSERT INTO saved_jobs (user_id, application_id) VALUES (%s, %s)" if _use_postgres else "INSERT INTO saved_jobs (user_id, application_id) VALUES (?, ?)",
            (user_id, app_id),
        )
        conn.commit()
        return app_id
...
def get_saved_applications(user_id: int) -> List[Dict[str, Any]]:
    ...
    cur = _cursor(conn)
    cur.execute(
        "SELECT a.*, s.saved_at FROM saved_jobs s JOIN applications a ON s.application_id = a.id WHERE s.user_id = %s ORDER BY s.saved_at DESC" if _use_postgres else "SELECT a.*, s.saved_at FROM saved_jobs s JOIN applications a ON s.application_id = a.id WHERE s.user_id = ? ORDER BY s.saved_at DESC",
        (user_id,),
    )
    return [_parse_app_row(_row_to_dict(r)) for r in cur.fetchall()]
...
def cleanup_old_saved_jobs(days: int = 7):
    ...
    cur = _cursor(conn)
    cur.execute(
        "SELECT s.application_id FROM saved_jobs s WHERE s.saved_at < %s" if _use_postgres
    ...
    cur.execute(
        "DELETE FROM saved_jobs WHERE saved_at < %s" if _use_postgres
...
def unsave_job(user_id: int, application_id: int) -> bool:
    ...
    cur = _cursor(conn)
    cur.execute(
        "DELETE FROM saved_jobs WHERE user_id = %s AND application_id = %s" if _use_postgres else "DELETE FROM saved_jobs WHERE user_id = ? AND application_id = ?",
        (user_id, application_id),
    )
    conn.commit()
    return cur.rowcount > 0
...
def get_saved_application_ids(user_id: int) -> set:
    conn = get_db()
    cur = _cursor(conn)
    cur.execute("SELECT application_id FROM saved_jobs WHERE user_id = %s" if _use_postgres else "SELECT application_id FROM saved_jobs WHERE user_id = ?", (user_id,))
...
def get_stats(user_id: Optional[int] = None) -> Dict[str, Any]:
    ...
    if user_id:
        cur.execute(
            "SELECT COUNT(*) as c FROM applications WHERE user_id = %s" if _use_postgres
            else "SELECT COUNT(*) as c FROM applications WHERE user_id = ?",
            (user_id,),
        )
        total = _row_to_dict(cur.fetchone())["c"]
        cur.execute(
            "SELECT COALESCE(AVG(ai_score), 0) as a FROM applications WHERE user_id = %s AND ai_score > 0" if _use_postgres
            else "SELECT COALESCE(AVG(ai_score), 0) as a FROM applications WHERE user_id = ? AND ai_score > 0",
            (user_id,),
        )
        avg = _row_to_dict(cur.fetchone())["a"]
        cur.execute(
            "SELECT COUNT(*) as c FROM applications WHERE user_id = %s AND ai_score >= 80" if _use_postgres
            else "SELECT COUNT(*) as c FROM applications WHERE user_id = ? AND ai_score >= 80",
            (user_id,),
        )
        high = _row_to_dict(cur.fetchone())["c"]
        cur.execute(
            "SELECT COUNT(*) as c FROM saved_jobs WHERE user_id = %s" if _use_postgres
        ...
        total = _row_to_dict(cur.fetchone())["c"]
        cur.execute("SELECT COALESCE(AVG(ai_score), 0) as a FROM applications WHERE ai_score > 0")
        avg = _row_to_dict(cur.fetchone())["a"]
