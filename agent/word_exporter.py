...
class WordExporter:
    ...
    def export_jobs_with_scores(
        self,
        scored_jobs: List[ScoredJob],
        profile_name: str = "",
        cv_texts: Optional[Dict[str, str]] = None,
        ...
        for range_label, filename in self.SCORE_RANGES:
            ...
            self._create_range_document(
                word_filepath, jobs, range_label, cv_text, profile_name
            )
            ...
            created_files.append(
                (range_label, str(word_filepath), len(jobs), str(pdf_filepath))
            )
    ...
    def _create_cv_pdf(
        self, filepath: Path, cv_text: str, profile_name: str, range_label: str
    ):
        ...
        pdf.cell(
            0, 10, txt=self._sanitize_for_pdf(profile_name or "CV"), ln=True, align="C"
        )
        ...
        pdf.cell(
            0,
            8,
            txt=self._sanitize_for_pdf(
                f"Tailored for {range_label}% Match Jobs -- Generated {datetime.now().strftime('%Y-%m-%d')}"
            ),
            ln=True,
            align="C",
        )
        ...
        for line in lines:
            ...
            is_bullet = stripped.startswith(("•", "-", "·", "*")) or re.match(
                r"^\d+[\.\)]\s+", stripped
            )
    ...
    def export_all_jobs(
        ...
        jobs: List[Job],
        profile_name: str = "",
        filename: str = "job_listings.docx",
    ):
        ...
        for job in jobs:
            platform_name = (
                job.platform.value.upper()
                if hasattr(job.platform, "value")
                else str(job.platform)
            )
...
def export_jobs_to_word(
    jobs: List[Job], profile_name: str = "", filepath: str = None
) -> str:
...
def export_scored_jobs_to_word(
    scored_jobs: List[ScoredJob],
    profile_name: str = "",
    output_dir: str = ".",
    cv_texts: Optional[Dict[str, str]] = None,
