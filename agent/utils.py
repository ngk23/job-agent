...
from .config import AppConfig, get_env
...
def setup_logging(
    name: str = __name__, data_dir: Optional[str] = None
) -> logging.Logger:
    ...
    logging.basicConfig(
        ...
        handlers=[logging.FileHandler(_LOG_DIR / "agent.log"), logging.StreamHandler()],
    )
...
class RateLimiter:
    ...
    async def wait(self, context: str = ""):
        ...
        if self.current_delay > self.base_delay:
            logger.info(
                f"Rate limit backoff: waiting {self.current_delay:.1f}s ({context})"
            )
    ...
    def failure(self):
        ...
        logger.warning(
            f"Failure #{self.failures}, delay increased to {self.current_delay:.1f}s"
        )
...
class ResumeHandler:
    ...
    def get_for_cover_letter(self) -> str:
        ...
        return (
            self.parsed_text[:1000] + "..."
            if len(self.parsed_text) > 1000
            else self.parsed_text
        )
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                ...
                logger.warning(
                    f"Retry {attempt + 1}/{max_retries} after {delay:.1f}s ({context}): {e}"
                )
